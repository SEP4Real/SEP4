...
<!-- ============================================================
  ...
  STRUCTURE (mandated by SEP4 requirements document):
    ...
    Design Overview              ← shared, includes cloud architecture
      ...
      ├─ ML Models               ┐
      ├─ DevOps and CI/CD        ├─ still branched by component
      └─ Verification            ┘
...
The domain model describes the problem space around students, study sessions, environmental observations, feedback, and planning. It identifies the concepts that matter to the users and stakeholders before implementation-specific decisions such as storage, API shape, or framework structure are introduced.

![Conceptual Domain Model of the StudyHelper System.](../Analysis/StudyHelper.domain.svg)

### Conceptual Breakdown

The domain is partitioned into logical areas that explain the real-world activities StudyHelper supports:

- **User and Profile**: A `User` represents a person using StudyHelper. A `UserProfile` captures relevant study context, such as university, study programme, study year, study goal, and profile picture.
- **Device**: A `Device` represents the physical StudyHelper unit that observes a study environment.
- **Session**: A `Session` represents a bounded period of studying. It starts when monitoring begins, ends when the student stops monitoring, and provides context for the environmental observations collected during that period.
- **DataPoint**: A `DataPoint` represents one observation of the study environment, including temperature, humidity, CO₂ level, light level, and the time of observation.
- **Rating**: A `Rating` represents the student's subjective assessment of the study session on a scale from 1 to 5. It captures perceived study suitability rather than objective cognitive performance.
- **CalendarEvent**: A `CalendarEvent` represents planned study time, allowing study sessions to be considered alongside the student's schedule.

### Analysis of Conceptual Modeling Decisions

1. **Sessions as the Core Context**: StudyHelper is concerned with environmental conditions during a study activity, not isolated sensor readings. Grouping observations under a `Session` makes the student's experience easier to interpret.
2. **Subjective Feedback as a Separate Concept**: Ratings are modeled separately from environmental observations because they describe the user's perception of the session. This distinction is important: two students may experience the same physical conditions differently.
3. **Profile and Planning as Supporting Context**: Profile and calendar information are included because study conditions are personal and time-bound, but they are not treated as replacements for session observations and ratings.
...
| ID   | User Story                                                                                                                                                                       |
| :--- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| US01 | As a Student, I want an overview of current environmental conditions and a suitability prediction, so that I can decide whether to start or continue a study session.            |
| US02 | As a Student, I want to submit a rating after a study session, so that the system can improve suitability predictions over time.                                                 |
| US03 | As a Student, I want to see the history of environmental conditions during my study session, so that I can understand trends affecting my focus.                                 |
| US04 | As a Student, I want to receive an alert when conditions fall below an acceptable threshold, so that I can take action to improve my study environment.                          |
| US05 | As a Student, I want to start and stop a monitored study session using a physical button on the device, so that I do not need to interact with any software to begin monitoring. |
| US06 | As a Teacher, I want an overview of current and predicted environmental conditions, so that I can decide if the environment is suitable for teaching.                            |
| US07 | As a Student, I want the web application to work correctly on my phone, tablet, and laptop, so that I can check conditions from any device.                                      |
| US08 | As a User, I want to register, log in, and log out securely, so that my personal data and study information are protected.                                                       |
| US09 | As a Student, I want to connect my physical device to my account, so that the dashboard can show data from my own study device.                                                  |
| US10 | As a Student, I want to manage my profile information, so that the application can be personalized to me.                                                                        |
| US11 | As a Student, I want to manage calendar events, so that I can plan study sessions in the same application.                                                                       |
| US12 | As a User, I want to switch between English and Danish and use dark mode, so that the application is easier to use in different contexts.                                        |
...
| NFR06 | The CI/CD pipeline must execute automated verification and block code merges if required builds or tests fail.                                                       | Non-functional | DevOps           |

## 2.4 System Sequence Diagrams {#24-system-sequence-diagrams}

System Sequence Diagrams (SSDs) were used to clarify the order of actions between actors and the StudyHelper system. Because the system behavior is relatively straightforward, the SSDs were most useful as a shared understanding aid rather than as a detailed design driver. The detailed use case descriptions, including actors, preconditions, flows of events, and postconditions, are provided in Appendix 2.2.

### 2.4.1 IoT-Initiated Session Lifecycles

The start and end use cases describe how a student begins and finishes a monitored study session.
...
The diagrams show the expected interaction at the system boundary: a student initiates a session, the system confirms that monitoring is active, and a later stop action ends the session. Preconditions and postconditions are documented in the corresponding use case descriptions.

### 2.4.2 Active Telemetry and Real-Time Alerting

During a running session, the system receives environmental observations and evaluates whether the conditions remain suitable for studying.
...
These SSDs show that condition assessment and alerting are part of the same user-facing activity: the system receives current study conditions and, when the predicted quality is critically poor, notifies the student. The implementation-specific timers, endpoints, and hardware alert mechanism are described later in the design and implementation sections.
...
The rating SSD captures the user-visible flow: the student submits a study suitability rating, and the system confirms that the feedback has been received. The detailed validation rules and UI behavior are design decisions described later.
...
The StudyHelper backend is hosted on a standard VPS owned by a group member and managed through Coolify, an open-source PaaS. Coolify coordinates container lifecycles and routes traffic to the Core API and frontend containers.
...
The initial data strategy was to collect StudyHelper sensor readings and pair them with user-provided session ratings. Because the project did not yet have enough live session data for training, the search was expanded to public environmental and comfort-related datasets. Early experiments considered noise-related data, but the project focus was narrowed from predicting internal cognitive focus to predicting a user-provided **Study Suitability Rating**. This shift was necessary because cognitive focus is a subjective internal state that cannot be directly measured by ambient sensors. User ratings provide a clearer target because they explicitly describe how suitable the user found the study environment.
...
![Correlation matrix from a rejected candidate dataset. The weak and inconsistent relationships between variables indicated that the data was unsuitable for reliable model training.](../ProcessReport/image/process-report/1778581896856.png)
...
Multi-language support (English and Danish) is implemented using React Context. Most page and component labels are loaded dynamically from centralized translation files. During testing, the team found that some navigation and top-bar controls still used hard-coded labels and did not switch language consistently. Light and dark theme configurations are applied globally using CSS variables. Both user preferences (language and theme) are persisted in `localStorage` to preserve configuration state across browser sessions.
...
Since the physical IoT prototype lacks a noise sensor, a conservative default of `noise = 29.0` is supplied by the Core API during inference. This baseline value was selected based on environmental studies indicating that ambient noise levels in empty educational buildings typically range between 25 and 30 dB(A) (Wiater & Gładyszewska-Fiedoruk, 2024). In the MAL service, the API-style payload fields are mapped to model columns (e.g., `currentNoise` to `noise_latest`, `meanNoise` to `noise_mean`).
...
The instant pipeline performs point-in-time predictions from a 5-feature vector: `temperature`, `humidity`, `co2`, `light`, and `noise`. If the client omits noise, the backend substitutes the baseline default value of 29.0, reflecting typical empty-room acoustic conditions (Wiater & Gładyszewska-Fiedoruk, 2024).

The training data is sourced from `MAL/data/processed/instant_mock_clean.csv`. Hyperparameters were explored through grid search for the main instant model and random search in related experiments where the parameter space was broader. The resulting production artifacts are `MAL/models/instantrfcmodel.pkl` and `MAL/models/instant_scaler.pkl`.
...
For the instant pipeline, the dataset was split into training/development (80%) and hold-out test (20%) sets. Hyperparameters were tuned using grid search and random search variants with cross-validation on the development set. For the GBC and MLP instant models, a room-grouped split (`GroupShuffleSplit`) was implemented to assess generalisation to unseen locations.
...
The session-based models were the more relevant result for the final system because the deployed workflow evaluates a running study session rather than a single isolated reading. Aggregating readings over time gave the models access to environmental stability and trends, which improved performance over the instant models. The Random Forest achieved the highest test accuracy, while the MLP was selected for deployment because it offered similar validation quality with a smoother prediction distribution for the API pipeline.

![Session-based MLP confusion matrix used to evaluate the selected full-session prediction workflow.](../../Documentation/Design/ML/cm_MLP_3.png){width=60%}
...
## 3.10 DevOps and CI/CD — Frontend {#38-frontend-ci-cd}
...
Localization tests verified the implemented translation mechanism, including default language rendering, language changes through React Context, and persistence through localStorage. They also exposed gaps: several navigation and top-bar controls still used fixed labels and did not switch language consistently.
...
## 3.13 Machine Learning Testing and MLOps
...
At requirements level, the prototype met the core monitoring, session lifecycle, rating, authentication, dashboard, and deployment requirements. The main partial items were security and localization: IoT communication remained plain HTTP because of microcontroller constraints, and the frontend language switch covered most content but not every top-level navigation/control label. The CI pipelines blocked failed builds or tests, but the project did not rely on a shared percentage coverage threshold across all components.
...
StudyHelper addressed the challenge of giving students real-time feedback about study-space conditions. The final prototype combined an IoT device measuring temperature, humidity, CO₂, and light; a Core API and PostgreSQL database for sessions and telemetry; a MAL service for suitability prediction; and a React frontend for live readings, history, and ratings. These components were deployed together through Docker Compose and Coolify.

The system met its main technical objectives. The IoT firmware transmitted readings every 30 seconds with keepalive pulses every five seconds, the backend persisted sessions and telemetry, the frontend displayed live and historical conditions across the required breakpoints, and CI pipelines verified builds and tests for the main components. The ML pipeline trained both instant and session-based models, with the best session-based model reaching 68.3% validation accuracy against a 57.9% majority-class baseline. These results are offline validation results, not live production accuracy.

The project partially solved the stated problem. Session-level environmental data can support useful study suitability guidance, but point-in-time readings alone were not reliable enough to predict subjective study quality. This confirmed the project's main limitation: comfort and focus are personal, so future versions need more real StudyHelper data and user-specific profiling. The security objective was also only partially met because IoT-to-backend communication remained plain HTTP.

# 6. Future Work

Future work should focus on strengthening the prototype rather than adding many unrelated features. The highest priority is collecting more real StudyHelper session data from actual users and the final IoT device. A larger dataset with repeated ratings per user would allow the ML component to move beyond general population averages and support personalization.

The second priority is improving operational reliability. Device-to-backend communication should be upgraded from plain HTTP where the hardware allows it, database migrations and backups should replace ad hoc schema initialization, and the deployment should gain a staging environment before production. More end-to-end browser and post-deployment tests would also reduce regression risk.

The frontend should address its remaining localization gaps, especially top-level controls that did not consistently switch between English and Danish. Device ownership should also be enforced more strongly so that users can manage one or more devices without accessing unrelated session data.

On the hardware side, future iterations should improve usability by adding easier Wi-Fi provisioning, more portable power, and a less disruptive alert option. A calibrated ambient noise sensor could also be reconsidered, but only if it provides reliable continuous measurements rather than occasional loud-sound detection.

Finally, the ML workflow should become more reproducible by versioning datasets, preprocessing outputs, model artifacts, and evaluation results together. Session-based prediction should remain the primary workflow, while instant prediction should be treated as an experimental warning signal unless more contextual and user-specific data becomes available.
...
Wiater, J., & Gładyszewska-Fiedoruk, K. (2024). Analysis of noise in education buildings. *Journal of Ecological Engineering, 25*(2), 176-181. https://doi.org/10.12911/22998993/176142
